...
for filename in silos:
    ...
    # Pick 5 random silos (excluding current)
    others = [s for s in silos if s != filename]
    picks = random.sample(others, min(5, len(others)))
...
"""
    # Replace existing explorer if it exists
    import re
    pattern = r'<!-- ======== PROJECT EXPLORER.*?-->\s*<section class="section related-silos".*?</section>'
    if re.search(pattern, content, re.DOTALL):
        content = re.sub(pattern, explorer_section.strip(), content, flags=re.DOTALL)
...
print("Circular Silo Expansion Complete.")
